#!/bin/bash
#
# Script para organizar fotos y videos, agrupando álbumes por año y
# reemplazando espacios en los nombres por guiones bajos.
#
...
process_video() {
    ...
    # === INICIO CAMBIO 3.1: Sanitizar nombre de archivo de video ===
    ...
    # El archivo temporal puede mantener el nombre original con espacios
    ...
    if ffmpeg -nostdin -i "$file" \
       -c:v libsvtav1 -crf 38 -preset 6 -svtav1-params "$svt_params" \
       -c:a copy -map_metadata 0 -movflags +faststart \
       -y "$output_file_temp" &> /dev/null; then

        echo "  -> Conversión de '$(basename "$file")' exitosa."
        
        # === INICIO CAMBIO 3.2: Mover el convertido con nombre sanitizado ===
        ...
        # === INICIO CAMBIO 3.3: Mover el original con nombre sanitizado ===
...
while IFS= read -r file; do
    ...
    if [ "$file_dir" == "$SOURCE_DIR" ]; then
        ...
        # === INICIO CAMBIO 2.1: Sanitizar nombre de album ===
        album_name_raw=$(basename "$file_dir")
    ...
    # === INICIO CAMBIO 2.2: Sanitizar nombre de archivo para la acción final ===
    filename_raw=$(basename "$file")
    filename_sanitized=${filename_raw// /_}
    ...
    if [ "$file_type" == "image" ]; then
        ...
        # La sanitización del nombre del video se maneja dentro de la función process_video
